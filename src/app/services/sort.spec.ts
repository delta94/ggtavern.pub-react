import { sortItems, sortItemsReverse } from './sort';

describe('Sort Service', () => {
    const sortTestData: TestData[] = [
        {
            test: 'objects-c',
            data: [
                { a: 'hello', b: 'really', c: 'fun' },
                { a: 'bye', b: 'ah', c: 'zed' }],
            sort: 'c',
            expected: [
                { a: 'hello', b: 'really', c: 'fun' },
                { a: 'bye', b: 'ah', c: 'zed' }
            ]
        },
        {
            test: 'objects-b',
            data: [
                { a: 'hello', b: 'really', c: 'fun' },
                { a: 'bye', b: 'ah', c: 'zed' }
            ],
            sort: 'b',
            expected: [
                { a: 'bye', b: 'ah', c: 'zed' },
                { a: 'hello', b: 'really', c: 'fun' }]
        },
        {
            test: 'objects-a',
            data: [
                { a: 'hello', b: 'really', c: 'fun' },
                { a: 'bye', b: 'ah', c: 'zed' }
            ],
            sort: 'a',
            expected: [
                { a: 'bye', b: 'ah', c: 'zed' },
                { a: 'hello', b: 'really', c: 'fun' }
            ]
        }
    ];

    sortTestData.forEach(test => {
        it(`should sort ${test.test}`, () => {
            const returned = sortItems(test.data, test.sort);
            expect(returned).toEqual(test.expected);
        });

    });
    it('should not sort incongruous items', () => {
        const returned = sortItems([{ twi: 'whoo' }, { la: 'yay' }, { bob: 'not a god' }], 'bob');
        expect(returned).toEqual([{ twi: 'whoo' }, { la: 'yay' }, { bob: 'not a god' }]);
    });

    it('should return items on failed sort', () => {
        spyOn(Array.prototype, 'sort').and.throwError('oops');
        const items = sortItems([1, 2, 3], null);
        expect(items).toEqual([1, 2, 3]);
    });
})

interface TestData {
    test: string;
    data: TestSortData[];
    sort: keyof TestSortData;
    expected: TestSortData[];
}
interface TestSortData {
    a: string;
    b: string;
    c: string;
}
